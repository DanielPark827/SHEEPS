...
class TeamChatPage extends StatefulWidget {
  ...
  TeamChatPage({Key key, this.roomName, this.titleName, this.chatUserList}) : super(key : key);
...
class _TeamChatPageState extends State<TeamChatPage> {
  double sizeUnit = 1;
  String settingIconName = 'assets/images/Chat/settingIcon.svg';
  ...
  void initState() {
    ...
    if(widget.chatUserList.length == null)
    ...
    if(null != _chatGlobal.getRoomInfoList){
      ...
      for(int i = 0; i < _chatGlobal.getRoomInfoList.length; ++i){
        ...
        for(int j = 0 ; j < _chatGlobal.getRoomInfoList[i].chatList.length; ++j){
          ...
          // //변경된 날짜 체크
          // if(j != 0){
          //   ChatRecvMessageModel prevMessage = _chatGlobal.getRoomInfoList[i].chatList[j-1];
          //   String updateDays = getYearMonthDayByString(prevMessage.updatedAt);
          //
          //   //마지막으로 읽은 것과 현재 날짜가 다른지 체크
          //   if(int.parse(updateDays) != int.parse(getYearMonthDayByDate())) {
          //     dateList.add(j);
          //   }
          // }
        ...
        //       message: DateTime.now().year.toString() + "년 " + DateTime.now().month.toString() + "월 " + DateTime.now().day.toString() + "일",
        //       isImage: 0,
        //       date: null,
        //       isRead: 1
        //   );
        //
        //   _addRecvMessage(0, chatRecvMessageModel, dateList[i], true);
        // }
          ...
          itemBuilder: (context, index) {
            ...
            return Container(
              ...
              child: ChatItem(
                ...
                chatIconName: GlobalProfile.getUserByUserID(_chatGlobal.getRoomInfoList[roomIndex].chatList[index].from) == null ? '' : GlobalProfile.getUserByUserID(_chatGlobal.getRoomInfoList[roomIndex].chatList[index].from).profileUrlList[0],
              ),
            );
          },
        ),
          ...
          GestureDetector(
            onTap: () async {
              ...
              for(int i = 0 ; i < widget.chatUserList.length; ++i){
                ...
                userListStr += widget.chatUserList[i].userID.toString();
              }
                  ...
                  Flexible(
                    child: TextField(
                      ...
                      decoration: InputDecoration (
                        ...
                        contentPadding: EdgeInsets.symmetric(horizontal: 12*sizeUnit, vertical: 10*sizeUnit),
          ...
          child: SafeArea(
            child: RefreshIndicator(
                  ...
                  for(int i = 0 ; i < chatList.length; ++i){
                    ...
                    if (cnt != 0) {
                      bool isContinue = (chatRecvMessageModel.from ==
                          _chatGlobal.getRoomInfoList[roomIndex].chatList[cnt - 1].from) &&
                          (chatRecvMessageModel.date ==
                              _chatGlobal.getRoomInfoList[roomIndex].chatList[cnt - 1].date);
                      if (true == isContinue) {
              ...
              child: GestureDetector(
                ...
                child: Scaffold(
                    appBar: SheepsAppBar(context, widget.titleName,
                      backFunc: (){
                        ChatGlobal.sortRoomInfoList();
                        ChatGlobal.currentRoomIndex = -1;
                        Navigator.pop(context, 'HotReload');
