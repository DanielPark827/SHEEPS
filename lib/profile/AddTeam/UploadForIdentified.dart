...
import 'package:sheeps_app/profile/AddTeam/UploadCompleteForIdentified.dart';
import 'package:sheeps_app/profile/models/ModelAddTeam.dart';
import 'package:sheeps_app/config/GlobalWidget.dart';
import 'package:sheeps_app/config/UploadUI.dart';

class UploadForIdentified extends StatefulWidget {
  @override
  _UploadForIdentifiedState createState() => _UploadForIdentifiedState();
}

class _UploadForIdentifiedState extends State<UploadForIdentified> {
  @override
      ...
      child: Scaffold(
        ...
        appBar: SheepsAppBar(
          ...
          backFunc: () {
            if (_ModifiedTeamProfile.FlagForIdentified == true) {
              ...
              if (_ModifiedTeamProfile.ProjectAgency != null &&
                  _ModifiedTeamProfile.ProjectStart != null &&
                  _ModifiedTeamProfile.ProjectEnd != null &&
                  _ModifiedTeamProfile.ProjectName != null) {
              ...
              if (_ModifiedTeamProfile.TeamAwardGrade != null &&
                  _ModifiedTeamProfile.TeamAwardAgency != null &&
                  _ModifiedTeamProfile.TeamAwardTime != null &&
                  _ModifiedTeamProfile.TeamAwardName != null) {
        ...
        body: UploadBody(
          ...
          cancelFileChangeFunc: () {
            _ModifiedTeamProfile.removeFile(
                _ModifiedTeamProfile.IdentifiedFile[
                    _ModifiedTeamProfile.IdentifiedFile.length - 1],
                1);
          },
          ...
          cameraFunc: () async {
            ...
            if (_ModifiedTeamProfile.FlagForIdentified == true) {
              if (_ModifiedTeamProfile.IdentifiedList.length <
                  _ModifiedTeamProfile.IdentifiedFile.length) {
                _ModifiedTeamProfile.removeEndFile(1);
              }
              _ModifiedTeamProfile.addFiles(File(f.path), 1);
            } else if (_ModifiedTeamProfile.FlagForProject == true) {
              if (_ModifiedTeamProfile.ProjectList.length <
                  _ModifiedTeamProfile.ProjectFile.length) {
                _ModifiedTeamProfile.removeEndFile(2);
              }
              _ModifiedTeamProfile.addFiles(File(f.path), 2);
            } else if (_ModifiedTeamProfile.FlagForTeamAward == true) {
              if (_ModifiedTeamProfile.TeamAwardList.length <
                  _ModifiedTeamProfile.AwardFile.length) {
            ...
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        UploadCompleteForIdentified()) // SecondRoute를 생성하여 적재
                ).then((value) {
              Navigator.pop(context);
              Navigator.pop(context);
            });
