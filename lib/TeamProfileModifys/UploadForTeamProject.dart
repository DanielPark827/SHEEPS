...
import 'package:sheeps_app/TeamProfileModifys/UploadCompleteForTeamProject.dart';
...
class UploadForTeamProject extends StatefulWidget {

  @override
  _UploadForIdentifiedState createState() => _UploadForIdentifiedState();
}

class _UploadForIdentifiedState extends State<UploadForTeamProject> {
  ...
  Widget build(BuildContext context) {
    ...
    return MediaQuery(
      ...
      child: Scaffold(
        ...
        body: UploadBody(
          context,
          file: _ModifiedTeamProfile.ProjectFile.length == 0 ? null : _ModifiedTeamProfile.ProjectFile[_ModifiedTeamProfile.ProjectFile.length-1],
          cancelFileChangeFunc: () {
            _ModifiedTeamProfile.removeFile(_ModifiedTeamProfile.ProjectFile[_ModifiedTeamProfile.ProjectFile.length-1],2);
          },
          ...
          cameraFunc: () async {
            ...
            _ModifiedTeamProfile.ChangeIfAddProjectUploadComplete(true);
            Navigator.push(
                context, // 기본 파라미터, SecondRoute로 전달
                MaterialPageRoute(
                    builder: (context) =>
                        UploadCompleteForTeamProject()) // SecondRoute를 생성하여 적재
            ).then((value) {
              Navigator.pop(context);
              Navigator.pop(context);
            });
